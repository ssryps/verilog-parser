...
parameter BASE_RAM_INIT_FILE = "G:/cod19grp9/kernel/kernel_trivial.bin"; //BaseRAM初始化文件，请修改为实际的绝对路?
parameter EXT_RAM_INIT_FILE = "G:/tmp/kernel_exp.bin";    //ExtRAM初始化文件，请修改为实际的绝对路?
//parameter FLASH_INIT_FILE = "/tmp/kernel.elf";    //Flash初始化文件，请修改为实际的绝对路?
//parameter BASE_RAM_INIT_FILE = "H:/cpu11/cod19grp9-master/kernel/kernel(2).bin"; //BaseRAM初始化文件，请修改为实际的绝对路?
//parameter EXT_RAM_INIT_FILE = "H:/cpu11/cod19grp9-master/kernel/kernel(2).bin";    //ExtRAM初始化文件，请修改为实际的绝对路?
parameter FLASH_INIT_FILE = "/tmp/kernel.elf";    //Flash初始化文件，请修改为实际的绝对路?
...
initial begin 
    ...
    $display("BaseRAM Init Size(words): %d",n_Init_Size);
    for (integer i = 0; i < n_Init_Size; i++) begin
        base.mem0[i] = tmp_array[i][24+:8];
        base.mem1[i] = tmp_array[i][16+:8];
        base.mem2[i] = tmp_array[i][8+:8];
        base.mem3[i] = tmp_array[i][0+:8];
    end
end
...
initial begin 
    ...
    $display("extRAM Init Size(words): %d",n_Init_Size);
    for (integer i = 0; i < n_Init_Size; i++) begin
        ext.mem0[i] = tmp_array[i][24+:8];
        ext.mem1[i] = tmp_array[i][16+:8];
        ext.mem2[i] = tmp_array[i][8+:8];
        ext.mem3[i] = tmp_array[i][0+:8];
    end
end
